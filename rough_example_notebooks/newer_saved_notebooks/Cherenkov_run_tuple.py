...
# Directory containing this script; used for relative paths (e.g. BC_PDE.csv)
...
def read_subprocess_text_to_DF(name_of_output_file:str):
    ...
    except ParserError: #pyarrow.lib.ArrowInvalid:
...
def create_root_bash_script(root_file_relative_path):
    ...
    root_thisroot_sh = os.environ.get("ROOT_THISROOT_SH")
    if not root_thisroot_sh:
        raise RuntimeError(
            "ROOT not configured for this code path. Set ROOT_THISROOT_SH to the path of thisroot.sh "
            "(e.g. export ROOT_THISROOT_SH=/path/to/root/bin/thisroot.sh). "
            "Note: multi-radiator runs read ROOT files via uproot in Python and do not require the ROOT binary."
        )

    with open(name_of_bash_script,"w") as root_script:
        root_script.write('#!/bin/bash\n')
        root_script.write(f'source {root_thisroot_sh}\n')
...
_bc_pde_path = os.path.join(_SCRIPT_DIR, "BC_PDE.csv")
_bc_pde_csv = os.environ.get("BC_PDE_CSV", _bc_pde_path)
if not os.path.isfile(_bc_pde_csv):
    raise FileNotFoundError(
        f"BC_PDE.csv not found at {_bc_pde_csv}. "
        "Ensure BC_PDE.csv is in the same directory as Cherenkov_run_tuple.py, or set environment variable "
        "BC_PDE_CSV to the full path of your BC_PDE.csv file."
    )
photodetector_efficiency = np.loadtxt(_bc_pde_csv, delimiter=',', dtype='float')
...
class multi_Cherenkov_run_tuple(Cherenkov_run_tuple):
    ...
    def __init__(self, file_path:str, 
        ...
        self.hits, self.output_tuples, self.coincidence_tuple_dictionary = self.construct_hits_dictionary(file_path, do_coincidence_analysis)

    def construct_hits_dictionary(self, file_path, do_coincidence_analysis):
        ...
        if do_coincidence_analysis is True:
