...
import numpy as np
from Cherenkov_run_tuple import Cherenkov_run_tuple, multi_Cherenkov_run_tuple, read_root_file_photon_generation
...
# Warn on import if GRAS/Geant4 env vars are not set (so users see it before running GRAS cells)
if not os.environ.get("GEANT4_SH") or not os.environ.get("GRAS_ENV_SH"):
    print(
        "\n*** Before running GRAS simulations: set GEANT4_SH and GRAS_ENV_SH. ***\n"
        "  export GEANT4_SH=/path/to/geant4/install/bin/geant4.sh\n"
        "  export GRAS_ENV_SH=/path/to/gras/config/gras-env.sh\n"
        "See README.md in this directory for details.\n"
    )

def run_gras_simulation():
    gras_env_sh = os.environ.get("GRAS_ENV_SH")
    if not gras_env_sh:
        raise RuntimeError(
            "GRAS environment not configured. Set GRAS_ENV_SH to the path of your gras-env.sh "
            "(e.g. export GRAS_ENV_SH=/path/to/gras/config/gras-env.sh)"
        )
    with open("gras_bash_script.sh","w") as gras_bash_script:
        gras_bash_script.write("#!/bin/bash\n")
        gras_bash_script.write(f"source {gras_env_sh}\n")
...
class gras_input_macro_file_generator():
    ...
    template_macro_file_path = "template_gras_input_macro.in"
...
class Cherenkov_gdml_geometry_generator():
    ...
    def __init__(self, aluminium_thickness_in_mm=0.0, tantalum_thickness_in_mm=0.0,radiator_geometry="cube"):
...
class gras_Cherenkov_runner():
    ...
    def __init__(self, 
        ...
        try:
            self.setup_input_variables(incoming_particles_per_s, verbose_output, aluminium_thickness_in_mm, tantalum_thickness_in_mm, radiator_geometry)
    ...
    def setup_input_variables(self, incoming_particles_per_s, verbose_output, aluminium_thickness_in_mm, tantalum_thickness_in_mm, radiator_geometry):
        self.incoming_particles_per_s = incoming_particles_per_s
        self.verbose_output = verbose_output
        self.generated_geometry = Cherenkov_gdml_geometry_generator(aluminium_thickness_in_mm,tantalum_thickness_in_mm,radiator_geometry=radiator_geometry)
    ...
    def initialise_running_directory_and_macro(self, number_of_particles, particle_type, spectrum_file_to_use, number_of_radiators=1):
        ...
        try:
            self.generated_macro = gras_input_macro_file_generator(number_of_particles, particle_type,gras_spectrum_macro(spectrum_file_to_use),gras_isotropic_macro())
        except Exception as failed_exception:
            os.chdir("..")
            raise failed_exception
    ...
    def initialise_output_gras_directory(self, spectrum_file_to_use, number_of_radiators=1):
        ...
        shutil.copyfile(spectrum_file_to_use,self.directory_to_run_in + spectrum_file_to_use)
    ...
    def initialise_dir_and_copy_geom_and_mac_files(self, number_of_radiators):
        ...
        if number_of_radiators == 1:
            template_gdml_file = "template_gdml_file_to_use_1_radiators.gdml"
            shutil.copyfile("Analysis.g4mac",self.directory_to_run_in + "Analysis.g4mac")
            shutil.copyfile("Analysis_reaction_m.g4mac",self.directory_to_run_in + "Analysis_reaction_m.g4mac")
            shutil.copyfile("Analysis_fluence_m.g4mac",self.directory_to_run_in + "Analysis_fluence_m.g4mac")
        elif number_of_radiators == 2:
            template_gdml_file = "template_gdml_file_to_use_2_radiators.gdml"
            shutil.copyfile("Analysis_2_radiators.g4mac",self.directory_to_run_in + "Analysis.g4mac")
            shutil.copyfile("Analysis_reaction_m.g4mac",self.directory_to_run_in + "Analysis_reaction_m.g4mac")
            shutil.copyfile("Analysis_fluence_m.g4mac",self.directory_to_run_in + "Analysis_fluence_m.g4mac")

        shutil.copyfile(template_gdml_file,self.directory_to_run_in + "template_gdml_file_to_use.gdml")
        shutil.copyfile("template_gras_input_macro.in",self.directory_to_run_in + "template_gras_input_macro.in")
    ...
    def calculate_Cherenkov_tuple(self, number_of_particles, number_of_radiators,**kwargs):
        ...
        if number_of_radiators == 1:
            ...
            try:
                outputted_tuples = multi_Cherenkov_run_tuple(file_path="output_Cherenkov_data.root", 
                                                        incoming_particles_per_second=self.incoming_particles_per_s,
                                                        number_of_events_simulated=number_of_particles,
                                                        do_coincidence_analysis=False,
                                                        **kwargs).output_tuples #["RA1_tuple_interaction;1"]
                #print("outputted_tuples is",outputted_tuples)
                outputted_tuple = outputted_tuples["FL1_tuple_fluence;1_CK"]
            except KeyError:
                print("KeyError: FL1_tuple_fluence;1_CK not found in output_tuples - probably because no counts were registered")
                columns_for_data = [
                ...
                outputted_tuple = Cherenkov_run_tuple(DF_of_output_hits=pd.DataFrame(columns=columns_for_data),
                                    incoming_particles_per_second=self.incoming_particles_per_s,
                                    number_of_events_simulated=number_of_particles,
                                    **kwargs)
        else:
    ...
    def run_gras_simulation(self):
        geant4_sh = os.environ.get("GEANT4_SH")
        gras_env_sh = os.environ.get("GRAS_ENV_SH")
        if not geant4_sh:
            raise RuntimeError(
                "Geant4 not configured. Set GEANT4_SH to the path of your geant4.sh "
                "(e.g. export GEANT4_SH=/path/to/geant4/install/bin/geant4.sh)"
            )
        if not gras_env_sh:
            raise RuntimeError(
                "GRAS not configured. Set GRAS_ENV_SH to the path of your gras-env.sh "
                "(e.g. export GRAS_ENV_SH=/path/to/gras/config/gras-env.sh)"
            )
        with open("gras_bash_script.sh","w") as gras_bash_script:
            gras_bash_script.write("#!/bin/bash\n")
            gras_bash_script.write(f"source {geant4_sh}\n")
            gras_bash_script.write(f"source {gras_env_sh}\n")
